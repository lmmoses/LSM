data { 
	...
	int<lower=1, upper=K> ant[M,N];						// MxN matrix of data, note the code cannot handle zero values
	int<lower=1, upper=K> ret[M,N];						// MxN matrix of data, note the code cannot handle zero values
...
parameters {
    ...
    real<lower=machine_precision(), upper=1> sigma;		// Noise in the data
			...
			if(i!=j) {
				for(k in 1:K+1){
					if(k==1){
						...
						phi[i,j,k] <- Phi((b[k-1] + source[i] + target[j])/sigma); // Phi is Normal(x;0,1) CDF
					}
				}
			}
		}
...
model {
	...
	for(i in 1:M) { 
		for(j in 1:N) {
			if (i!=j && !w[i,j]) {
				ant[i,j] ~ categorical(f[i,j]);
				ret[i,j] ~ categorical(f[i,j]);
			}
		}	
	}
} 
